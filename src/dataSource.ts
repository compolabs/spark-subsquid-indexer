...
import { BN, type ReceiptLogData, ReceiptType, getDecodedLogs } from "fuels";
...
export async function processBlocks(ctx: any) {
	...
	for (const block of blocks) {
		for (const receipt of block.receipts) {
			...
			if (
				MARKETS.includes(receipt.contract) &&
				tx.status.type == "SuccessStatus"
			) {
				receipts.push({
