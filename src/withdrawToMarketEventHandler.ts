...
export async function handleWithdrawToMarketEvent(
	log: WithdrawToMarketEventOutput,
	receipt: any,
	withdrawToMarketEvents: Map<string, any>,
	...
	if (balance) {
		...
		ctx.log.warn(
			`NO BALANCE WITHDRAW_TO FOR USER: ${getIdentity(log.user)} BALANCE ID: ${getHash(`${getIdentity(log.user)}-${receipt.id}`)} MARKET: ${receipt.id}.`,
		);
	}
}
