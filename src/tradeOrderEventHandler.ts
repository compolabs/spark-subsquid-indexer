...
export async function handleTradeOrderEvent(
	log: TradeOrderEventOutput,
	receipt: any,
	tradeOrderEvents: Map<string, any>,
	orders: Map<string, any>,
	activeBuyOrders: Map<string, any>,
	activeSellOrders: Map<string, any>,
	...
	const sellActiveOrder = assertNotNull(
		await lookupSellOrder(ctx.store, activeSellOrders, log.base_sell_order_id),
	);
	const sellOrder = assertNotNull(
		await lookupOrder(ctx.store, orders, log.base_sell_order_id),
	);
	const buyActiveOrder = assertNotNull(
		await lookupBuyOrder(ctx.store, activeBuyOrders, log.base_buy_order_id),
	);
	const buyOrder = assertNotNull(
		await lookupOrder(ctx.store, orders, log.base_buy_order_id),
	);

	// Retrieve the balances for both the seller and the buyer
	const seller_balance = assertNotNull(
		await lookupBalance(
			ctx.store,
			balances,
			getHash(`${getIdentity(log.order_seller)}-${receipt.id}`),
		),
	);
	const buyer_balance = assertNotNull(
		await lookupBalance(
			ctx.store,
			balances,
			getHash(`${getIdentity(log.order_buyer)}-${receipt.id}`),
		),
	);
	...
	sellOrder.status =
		updatedSellAmount === 0n ? OrderStatus.Closed : OrderStatus.Active;
	...
	sellActiveOrder.status =
		updatedActiveSellAmount === 0n ? OrderStatus.Closed : OrderStatus.Active;
	...
	buyOrder.status =
		updatedBuyAmount === 0n ? OrderStatus.Closed : OrderStatus.Active;
	...
	buyActiveOrder.status =
		updatedActiveBuyAmount === 0n ? OrderStatus.Closed : OrderStatus.Active;
	...
	if (seller_balance) {
		...
		ctx.log.warn(
			`NO BALANCE TRADE FOR USER: ${getIdentity(log.order_seller)} BALANCE ID: ${getHash(`${getIdentity(log.order_seller)}-${receipt.id}`)} MARKET: ${receipt.id}.`,
		);
	}
	...
	if (buyer_balance) {
		...
		ctx.log.warn(
			`NO BALANCE TRADE FOR USER: ${getIdentity(log.order_buyer)} BALANCE ID: ${getHash(`${getIdentity(log.order_buyer)}-${receipt.id}`)} MARKET: ${receipt.id}.`,
		);
	}
}
