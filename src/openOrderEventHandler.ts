...
export async function handleOpenOrderEvent(
	log: OpenOrderEventOutput,
	receipt: any,
	openOrderEvents: Map<string, any>,
	...
	const balance = assertNotNull(
		await lookupBalance(ctx.store, balances, balanceId),
	);
	...
	if (balance) {
		...
		ctx.log.warn(
			`NO BALANCE OPEN FOR USER: ${getIdentity(log.user)} BALANCE ID: ${getHash(`${getIdentity(log.user)}-${receipt.id}`)} MARKET: ${receipt.id}.`,
		);
	}
}
