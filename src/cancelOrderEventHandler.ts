...
export async function handleCancelOrderEvent(
	log: CancelOrderEventOutput,
	receipt: any,
	cancelOrderEvents: Map<string, any>,
	...
	const order = assertNotNull(
		await lookupOrder(ctx.store, orders, log.order_id),
	);
	const balance = assertNotNull(
		await lookupBalance(
			ctx.store,
			balances,
	...
	if (balance) {
		...
		ctx.log.warn(
			`NO BALANCE CANCEL FOR USER: ${getIdentity(log.user)} BALANCE ID: ${getHash(`${getIdentity(log.user)}-${receipt.id}`)} MARKET: ${receipt.id}.`,
		);
	}
}
